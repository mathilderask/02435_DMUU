# -*- coding: utf-8 -*-
"""
Diagnostic plotting script for DummyPolicy.py.

Run from the Assignment_B folder or any other folder. The script resolves all
data paths relative to this file.

This script simulates one full day with the dummy policy and saves a
Part-A-style plot to the Plots/ folder.
"""
...
def plot_results(sol: Dict[str, np.ndarray], price: np.ndarray, occ1: np.ndarray, occ2: np.ndarray, d: int) -> None:
    ...
    axes[0].set_title(f"Dummy Policy: Room Temperatures for Day {d + 1}", fontsize=title_fs)
    ...
    axes[1].set_title(f"Dummy Policy: Heater Consumption for Day {d + 1}", fontsize=title_fs)
    ...
    axes[2].set_title(f"Dummy Policy: Ventilation Status and Humidity for Day {d + 1}", fontsize=title_fs)
    ...
    fig.savefig(plots_dir / f"dummy_policy_day_{d + 1}.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    print(f"Saved plot: Plots/dummy_policy_day_{day + 1}.png")