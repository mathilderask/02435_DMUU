...
def plot_HVAC_results(HVAC_results):
    ...
    # Extract series from HVAC_results
    Temp_r1 = np.array(HVAC_results["T1"][:10])
    Temp_r2 = np.array(HVAC_results["T2"][:10])
    h_r1 = np.array(HVAC_results["p1"])
    h_r2 = np.array(HVAC_results["p2"])
    v = np.array(HVAC_results["v"])
    Hum = np.array(HVAC_results["H"][:10])
    price = np.array(HVAC_results["price"])
    Occ_r1 = np.array(HVAC_results["occ1"])
    Occ_r2 = np.array(HVAC_results["occ2"])
