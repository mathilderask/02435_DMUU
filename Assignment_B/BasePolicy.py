...
def _load_params() -> Dict[str, Any]:
    """Load SystemCharacteristics if available; otherwise use safe fallbacks.

    The exact key names may differ between course releases, so this function
    accepts several common aliases.
    """
    try:
        module = __import__("Data.v2_SystemCharacteristics", fromlist=["*"])
...
def _p(*names: str, default: Any) -> Any:
    """Return the first matching parameter name from SystemCharacteristics."""
    for name in names:
        if name in PARAMS:
            return PARAMS[name]
    return default
...
# Conservative defaults; overwritten automatically if SystemCharacteristics exists.
