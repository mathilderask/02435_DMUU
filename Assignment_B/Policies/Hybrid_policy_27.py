...
from . import ADP_policy_27 as ADP
...
def _print_runtime_summary() -> None:
    if not PRINT_RUNTIME or not _RUNTIME_LOG:
        return

    print(
        f"Hybrid rollout runtime summary | steps={len(_RUNTIME_LOG)} | "
        f"min={min(_RUNTIME_LOG):.2f}s | max={max(_RUNTIME_LOG):.2f}s",
        flush=True,
    )


atexit.register(_print_runtime_summary)
