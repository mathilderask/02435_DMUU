...
def select_action(state):
    ...
    # -----------------------------------------------------
    # Objective: SP cost + ADP-style terminal value
    # -----------------------------------------------------
    ...
    energy_cost = sum(
     ...
     # ADP component: approximate future cost-to-go at the leaves
    m.term_low_T = pyo.Var(m.L, m.R, within=pyo.NonNegativeReals)
    m.term_high_T = pyo.Var(m.L, m.R, within=pyo.NonNegativeReals)
    m.term_high_H = pyo.Var(m.L, within=pyo.NonNegativeReals)

    m.terminal_value_constraints = pyo.ConstraintList()
    ...
    terminal_value = sum(
        m.q[n]
        * (
            GAMMA_T_LOW * (m.term_low_T[n, 1] + m.term_low_T[n, 2])
            + GAMMA_T_HIGH * (m.term_high_T[n, 1] + m.term_high_T[n, 2])
            + GAMMA_H * m.term_high_H[n]
        )
        for n in leaf_nodes
    )

    m.obj = pyo.Objective(expr=energy_cost + terminal_value, sense=pyo.minimize)
